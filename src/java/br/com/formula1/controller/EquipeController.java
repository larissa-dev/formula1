...
public class EquipeController implements Serializable{
    ...
    public String salvar(){
        if (equipeService.inserir(equipe)){
            UtilMensagens.mensagemSucesso("Sucesso", "Equipe salvo com sucesso!");
            this.listar();
            return "list.xhtml?faces-redirect=true";
        }
        UtilMensagens.mensagemErro("Erro", "Ocorreu um erro ao salvar o equipe!");
        return null;
    }
    
    public String alterar(){
        if (equipeService.alterar(equipe)){
            UtilMensagens.mensagemSucesso("Sucesso", "Equipe alterado com sucesso!");
            this.listar();
            return "list.xhtml?faces-redirect=true";
        }
        UtilMensagens.mensagemErro("Erro", "Ocorreu um erro ao alterar o equipe");
        return null;
    }
    
    public String excluir(Equipe equipe){
        if (equipeService.excluir(equipe)){
            UtilMensagens.mensagemSucesso("Sucesso", "Equipe excluído com sucesso!");
            this.listar();
            return "list.xhtml?faces-redirect=true";
        }
        UtilMensagens.mensagemErro("Erro", "Ocorreu um erro ao excluir o equipe");
        return null;
    }
